...
class PID:
	...
	def vel_update(self):
		...
		if self.ed <= 0.1:
			...
			self.et_old = self.et
			if self.v > 0:
				self.motion.linear.x = min(self.v, 0.17)
			else:
				self.motion.linear.x = max(self.v, -0.17)
			if self.w > 1.5:
				self.w = 1.5
			if self.w < -1.5:
				self.w = -1.5
...
class getPathList:
    ...
    def __init__(self):
        self.sub = rospy.Subscriber("/planned_path", NextPoint, self.getPath)
        self.path = []
        self.counter = 0
        self.x = 0
